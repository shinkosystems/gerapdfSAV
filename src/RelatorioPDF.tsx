// src/RelatorioPDF.tsx - CÓDIGO FINAL COMPLETO
...
const styles = StyleSheet.create({
    page: { paddingHorizontal: 30, paddingTop: 146, paddingBottom: 50, backgroundColor: '#FFFFFF' }, 
    headerContainer: {
        flexDirection: 'row',
    ...
    return (
        <Page size="A4" style={styles.coverPage}>
            ...
            <View style={styles.coverDetailsBlock}>
                ...
                {/* NOVO CAMPO: Responsável pelo projeto */}
                <Text style={styles.coverDetail}>
                    <Text style={styles.coverDetailBold}>Responsável pelo projeto:</Text> {data.responsavel}
                </Text>
                {nrsList.length > 0 && (
...
// COMPONENTE: Cabeçalho Fixo (Mantido)
...
const RelatorioPDF = ({ data }: { data: RelatorioData }) => {
    ...
    // 2. COMPONENTE AUXILIAR 2: ItemRelatorio (CORREÇÃO FINAL DE EXIBIÇÃO)
    const ItemRelatorio = ({ item, forceNoBreak = false }: { item: ItemAuditoria, forceNoBreak?: boolean }) => (
        ...
        >
            ...
            <View style={styles.itemContent}>
                ...
                {/* CORREÇÃO FINAL APLICADA: Concatenação explícita de strings e tags para máxima compatibilidade */}
                {item.acompanhante && (
                    <Text style={{ fontSize: 10, marginBottom: 3 }}>
                        <Text style={{ fontWeight: 'bold' }}>{'Acompanhante: '}</Text>{item.acompanhante}
                    </Text>
                )}
